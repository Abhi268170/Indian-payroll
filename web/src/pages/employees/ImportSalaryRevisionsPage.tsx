...
export default function ImportSalaryRevisionsPage(): ReactElement {
  ...
  const validate = useCallback(async (file: File, overwriteExisting: boolean): Promise<void> => {
    ...
    try {
      ...
      const res = await api.post<ValidationResult>('/api/v1/salary-revisions/import/validate', form)
  ...
  const onImport = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const res = await api.post<CommitResult>('/api/v1/salary-revisions/import/commit', form)
  ...
  const readyCount = state.phase === 'validated' ? state.result.validRows.length : 0
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-[var(--color-text-primary)]">Import Salary Revisions</h1>
          <p className="text-sm text-[var(--color-text-secondary)] mt-0.5">
            Upload an XLSX file to create salary revisions for multiple employees at once.
          </p>
        </div>
        <button
          onClick={() => {
            void api.get<Blob>('/api/v1/salary-revisions/import/template', { responseType: 'blob' }).then(res => {
              const url = URL.createObjectURL(res.data)
              const a = document.createElement('a')
              a.href = url
              a.download = 'salary-revision-import-template.xlsx'
              a.click()
      ...
      {state.phase === 'done' && (
        <div className="bg-white rounded-lg border border-[var(--color-border)] p-10 text-center">
          ...
          <div className="text-lg font-semibold text-[var(--color-text-primary)] mb-1">Import Complete</div>
          <div className="text-sm text-[var(--color-text-secondary)] mb-6">
            {state.result.created > 0 && <span>{state.result.created} revision{state.result.created !== 1 ? 's' : ''} created. </span>}
            {state.result.skipped > 0 && <span>{state.result.skipped} skipped (duplicate payout). </span>}
          </div>
        ...
        <div className="bg-white rounded-lg border border-[var(--color-border)] p-6 space-y-5">
          {state.phase === 'idle' || state.phase === 'error' ? (
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-[var(--color-page-bg)] rounded-lg border border-[var(--color-border)]">
              ...
              {state.phase === 'validated' && (
                <button onClick={() => { setState({ phase: 'idle' }) }} className="text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)]">
                  <X size={16} />
                </button>
              )}
            </div>
          ...
          {(state.phase === 'validating' || state.phase === 'importing') && (
            <div className="text-sm text-[var(--color-text-secondary)] flex items-center gap-2">
              ...
              {state.phase === 'validating' ? 'Checking your file...' : 'Creating revisions...'}
            </div>
          )}
          ...
          {state.phase === 'validated' && (
            <div className="space-y-4">
              <div className="flex gap-3 flex-wrap">
                ...
                {state.result.skippedRows.length > 0 && (
                  <span className="inline-flex items-center px-3 py-1 bg-[var(--color-badge-grey-bg)] text-[var(--color-text-secondary)] text-sm font-medium rounded-full">
                    {state.result.skippedRows.length} will be skipped (duplicate payout)
                  </span>
                )}
              ...
              <div className="flex items-center gap-3 pt-1">
                ...
                >
                  Create {readyCount} {readyCount === 1 ? 'Revision' : 'Revisions'}
                </button>
                {hasErrors && (
                  <span className="text-xs text-[var(--color-text-muted)]">Fix all errors to enable import</span>
                )}
              </div>
            </div>
          )}
