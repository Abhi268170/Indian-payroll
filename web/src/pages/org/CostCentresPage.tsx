...
import type { CostCentreDto } from '@/types/api'
...
export default function CostCentresPage(): React.ReactElement {
  const qc = useQueryClient()
  const [showForm, setShowForm] = useState(false)
  const [apiError, setApiError] = useState<string | null>(null)

  const { data: costCentres, isLoading } = useQuery<CostCentreDto[]>({
  ...
  const create = useMutation({
    mutationFn: (v: FormValues) => api.post('/api/org/cost-centres', { name: v.name, code: v.code ?? null }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['cost-centres'] }); reset(); setShowForm(false); setApiError(null) },
    onError: () => setApiError('Failed to create cost centre.'),
  })

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-gray-900">Cost Centres</h1>
        <button onClick={() => setShowForm(f => !f)} className="bg-violet-600 text-white text-sm px-4 py-2 rounded-lg hover:bg-violet-700 transition-colors">
          {showForm ? 'Cancel' : '+ New Cost Centre'}
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit(v => create.mutate(v))} className="bg-white border border-gray-200 rounded-xl p-5 mb-6 space-y-4 max-w-md">
          ...
          <div>
            ...
            <input {...register('code')} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500" placeholder="e.g. CC-IT" />
          </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {costCentres?.length === 0 && <tr><td colSpan={2} className="px-4 py-6 text-center text-gray-400">No cost centres yet.</td></tr>}
              {costCentres?.map(c => (
                <tr key={c.id}>
